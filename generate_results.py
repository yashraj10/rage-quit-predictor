...
def load_model(weights_path="results/weights/best_model.pt"):
    ...
    model = RageQuitTransformer(**config)
    model.load_state_dict(checkpoint["model_state_dict"])
    model.eval()
    print(f"Loaded model from epoch {checkpoint['epoch']} (AUC-PR: {checkpoint['val_auc_pr']:.4f})")
    return model
...
def plot_confusion_matrix(labels, probs, save_dir="results/figures"):
...
def plot_model_comparison(transformer_metrics, save_dir="results/figures"):
    ...
    for idx, (name, vals) in enumerate(data.items()):
        ax = axes[idx]
        bars = ax.bar(models, vals, color=colors, width=0.55, edgecolor=[GRID_COLOR, VIOLET], linewidth=[0, 1.5])
        for bar, val in zip(bars, vals):
            ax.text(bar.get_x() + bar.get_width()/2, bar.get_height() + 0.008, f"{val:.3f}",
                    ha="center", va="bottom", fontsize=14, fontweight="bold", color=TEXT_PRIMARY)
        ax.set_title(name, fontsize=13, pad=10)
        ax.set_ylim(0, max(vals) * 1.25)
        ax.grid(axis="y", alpha=0.15); ax.set_axisbelow(True)
        winner = np.argmax(vals)
        if vals[0] != vals[1]:
            bars[winner].set_edgecolor(EMERALD); bars[winner].set_linewidth(2)
...
def plot_event_importance(event_importance, save_dir="results/figures"):
    ...
    bars = ax.barh(range(len(names)), values, color=bar_colors, height=0.65, alpha=0.85)
    ...
    if values:
        ax.annotate("← LESS LIKELY", xy=(min(values) * 0.6, len(names) + 0.3), fontsize=8, color=CYAN, ha="center", style="italic")
        ax.annotate("MORE LIKELY →", xy=(max(values) * 0.6, len(names) + 0.3), fontsize=8, color=ROSE, ha="center", style="italic")
...
def plot_summary_card(transformer_metrics, save_dir="results/figures"):
    ...
    for i, (val, label, color) in enumerate(stats):
        ...
        ax.add_patch(rect)
        ax.text(x+0.1,1.25,val,fontsize=20,ha="center",va="center",fontweight="bold",color=color,zorder=3)
        ax.text(x+0.1,0.6,label,fontsize=9,ha="center",va="center",color=TEXT_SECONDARY,zorder=3)
