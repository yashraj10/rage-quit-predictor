...
A production-grade operational layer built around the model — experiment tracking, GPU optimization, cloud storage, event streaming, drift monitoring, and pipeline orchestration.
...
| Pipeline Orchestration | Apache Airflow | 6-task weekly DAG: ingest → feature engineering → load → train → evaluate → drift check. XCom, SLA enforcement, retry logic | ✅ Implemented |
...
├── airflow_dag.py             # 6-task weekly retraining DAG (ingest → train → drift check)
...
- **Airflow DAG:** `@weekly` schedule, `max_active_runs=1`, XCom passes `mlflow_run_id` train → evaluate → drift
...
│   ├── airflow_dag.py          # 6-task weekly retraining DAG
│   └── quantization_results.json  # AMP + INT8 benchmark results
...
### 10. MLOps — Airflow DAG
```bash
pip install apache-airflow
cp mlops/airflow_dag.py ~/airflow/dags/
airflow standalone  # UI at localhost:8080
# DAG: rage_quit_weekly_retrain | Schedule: @weekly | Tasks: 6 | max_active_runs: 1
```
...
**Why Airflow for orchestration?** The pipeline has 6 sequential tasks with different failure modes — data tasks can retry, training cannot (expensive, needs human review). Airflow handles per-task retry logic, SLA enforcement, and XCom for passing run IDs between tasks. `@weekly` schedule with `max_active_runs=1` prevents concurrent retraining.
...
**Spark + SageMaker:** Architected in the MLOps design document but not yet deployed locally due to Docker/infrastructure requirements. These are the next layer of the production system.
